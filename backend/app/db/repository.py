...
def _seed_compilation(session, *, approved: bool = True) -> RuleCompilationRow:
    ...
    compilation = RuleCompilationRow(
        ...
        raw_response={
            **result.raw_response,
            "source_text": DEFAULT_CONTRACT_PATH.read_text(),
...
def _compilation_view(row: RuleCompilationRow | None) -> dict[str, object]:
    ...
    return {
        ...
        "diagnostics": raw_response.get("compiler_diagnostics", []),
        "clause_coverage": raw_response.get("clause_coverage", []),
        "approval_ready": bool(raw_response.get("approval_ready", True)),
