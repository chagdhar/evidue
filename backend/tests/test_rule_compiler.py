...
def test_unknown_operation_parameter_is_rejected():
    payload = json.loads(RECORDED_PROPOSAL_PATH.read_text())
    payload["rules"][0]["parameters"]["invented_parameter"] = True
    with pytest.raises(ValidationError, match="unsupported parameters"):
        CompilationProposal.model_validate(payload)


def test_unresolved_clause_requires_matching_blocking_diagnostic():
    payload = json.loads(RECORDED_PROPOSAL_PATH.read_text())
    payload["clause_coverage"] = [
        {
            "clause_id": "C-UNSUPPORTED",
            "clause_text": "The vendor receives a discretionary quarterly performance bonus.",
            "status": "needs_review",
            "rule_ids": [],
            "explanation": "No deterministic operation represents discretionary bonuses.",
        },
        *[
            {
                "clause_id": f"C-{rule['id']}",
                "clause_text": rule["clause_text"],
                "status": "compiled",
                "rule_ids": [rule["id"]],
                "explanation": "Represented by the referenced deterministic rule.",
            }
            for rule in payload["rules"]
        ],
    ]
    payload["diagnostics"] = []
    with pytest.raises(ValidationError, match="blocking diagnostic"):
        CompilationProposal.model_validate(payload)


def test_blocking_compilation_cannot_become_executable_program():
    payload = json.loads(RECORDED_PROPOSAL_PATH.read_text())
    clause = "The vendor receives a discretionary quarterly performance bonus."
    payload["clause_coverage"] = [
        {
            "clause_id": "C-UNSUPPORTED",
            "clause_text": clause,
    ...
    payload["diagnostics"] = [
        {
            "code": "UNSUPPORTED_BONUS_TERM",
            "severity": "blocking",
            "message": "A discretionary bonus cannot be compiled deterministically.",
            "clause_text": clause,
            "suggested_action": "Add an approved deterministic calculation or exclude this term.",
        }
    ]
    proposal = CompilationProposal.model_validate(payload)
    assert proposal.approval_ready is False
    with pytest.raises(ValueError, match="blocking or unresolved"):
        to_rule_program(
            proposal,
            compilation_id="TEST-BLOCKED",
            version=1,
