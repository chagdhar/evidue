...
function App() {
  ...
  return <Container maxWidth="lg"><header><Typography variant="h3">Evidue</Typography><Chip label="Synthetic demonstration data" color="warning"/><Typography>{disclosure}</Typography></header><Typography variant="h5">Acme Commerce × Nova Support AI · June 1–30, 2026</Typography><Typography className="promise">Every dollar is produced by deterministic rules evaluated against traceable evidence—not by a model's guess.</Typography><Card sx={{ my: 3 }}><CardContent><Typography variant="h6">Submitted invoice</Typography><Typography variant="h3">$15,000</Typography><Typography>10,000 claimed outcomes at $1.50. Contract rules and customer-owned evidence are ready for evaluation.</Typography><Button variant="contained" disabled={loading} onClick={run}>{loading ? "Applying deterministic rules…" : "Run reconciliation"}</Button>{error && <Typography color="error">{error}</Typography>}</CardContent></Card>{summary && <Card className="dominant"><CardContent><Typography>Correct payable amount</Typography><Typography variant="h2">{formatUsd(summary.payable_amount)}</Typography><Typography>{summary.payable_outcomes.toLocaleString()} payable outcomes · {summary.disputed_outcomes.toLocaleString()} disputed · {formatUsd(summary.recommended_deduction)} recommended deduction</Typography><Button href="/api/reconciliations/current/exports/disputes.csv">Download dispute CSV</Button><Button href="/api/reconciliations/current/exports/evidence.json">Download evidence JSON</Button><Button href="/api/reconciliations/current/exports/summary.json">Download summary JSON</Button></CardContent></Card>}</Container>;
}
