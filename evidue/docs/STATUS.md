...
## Pilot workflow wiring hardening — 2026-08-08

The protected pilot and finance surfaces received a focused state-machine and action-wiring hardening pass.

Fixed:

- the workflow can now recommend and mark the **Send & export** stage after a review-free reconciliation;
- manual stage navigation no longer permanently disables automatic guidance — guidance resumes when persisted backend progress changes the recommended stage;
- evidence is never considered ready merely because a verification plan has not loaded;
- invoice import now persists the automatic verification plan immediately, including the legitimate zero-external-evidence case;
- the Evidence stage exposes a recovery action to rebuild a missing verification plan instead of leaving the workflow stuck;
- disabled invoice-import and reconciliation actions now explain the blocking prerequisite next to the control;
- identity-review actions now fail visibly when no invoice/event/candidate is available, and manual confirmation cannot submit an empty match;
- finance dispute-PDF export failures are surfaced through the workspace error state;
- the finance workspace now shows explicit empty states for invoices, vendors, and review cases;
- the public read-only demo no longer exposes an enabled reconciliation button whose handler intentionally does nothing.

Regression coverage was added for evidence readiness, stage recommendation, and restoring automatic guidance after backend progress.

Validation in the handoff sandbox was limited by its internal dependency mirrors: npm could not retrieve `playwright-core@1.62.1` or `@emotion/react`, and uv could not retrieve FastAPI/SQLModel dependencies. TypeScript/TSX syntax parsing of every modified frontend file passed using the available TypeScript compiler, and Python `compileall` passed before generated caches were removed. Run `./scripts/check-all.sh` or `./scripts/dev-check.sh full` on the normal development machine before merging to `main`.
