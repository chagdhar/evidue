...
FORBIDDEN = [
    (re.compile(r"Y\s*Combinator", re.IGNORECASE), "Y Combinator reference"),
    (re.compile(r"\bYC\b"), "YC reference"),
    (re.compile(r"Evidue\s+Verify", re.IGNORECASE), "retired Evidue Verify brand"),
    (re.compile(r"Customer\s+Verify", re.IGNORECASE), "retired Customer Verify brand"),
]
violations = []
for root in SCANNED:
    paths = (
        ...
        if root.is_file()
            ...
            p
            for p in root.rglob("*")
            if p.is_file() and p.suffix in {".ts", ".tsx", ".css", ".md", ".html"}
        ]
    )
    for path in paths:
        if path in EXCLUDED:
