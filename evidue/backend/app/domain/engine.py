    ...
    *,
    ...
    for rule in sorted(approved.rules, key=lambda item: item.priority):
        ...
        if operation == "claim_amount_equals":
            value = Decimal(str(_claim_value(claim, str(p["claim_field"]))))
            expected = Decimal(str(p["expected_amount"]))
            tolerance = Decimal(str(p.get("tolerance", "0.00")))
            if abs(value - expected) > tolerance:
                return _rule_failure(
                    claim,
                    rule,
                    (
                        f"Billed amount {value:.2f} does not match the approved "
                        f"contract rate {expected:.2f}"
                    ),
