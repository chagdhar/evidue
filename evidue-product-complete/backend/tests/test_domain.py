...
def test_contract_rate_rule_disputes_an_overpriced_claim():
    from app.contracts.compiler import recorded_rule_program

    record = _payable_record()
    overpriced = replace(record.claim, billed_amount=Decimal("2.00"))
    result = evaluate(
        overpriced,
        list(events_for(overpriced, "payable")),
        program=recorded_rule_program(),
    )
    assert result.status == "disputed"
    assert result.rule_id == "R0"
    assert result.confirmed_disputed_amount == Decimal("2.00")
    assert "approved contract rate" in result.reason
